#import "FlutterFaiUpgradePlugin.h"
...
@interface FlutterFaiUpgradePlugin ()
...
@implementation FlutterFaiUpgradePlugin
...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar> *)registrar {
    [registrar addApplicationDelegate:[FlutterFaiUpgradePlugin shareInstance]];
    ...
    [FlutterFaiUpgradePlugin shareInstance].messageChannel = messageChannel;
    ...
    [messageChannel setMessageHandler:^(id message, FlutterReply callback) {
        ...
        if ([method isEqualToString:@"umInit"]) {
            ...
            if (@available(iOS 10.0, *)) {
                [UNUserNotificationCenter currentNotificationCenter].delegate = [FlutterFaiUpgradePlugin shareInstance];
            } else {
            // Fallback on earlier versions
            }
            [UMessage registerForRemoteNotificationsWithLaunchOptions:[FlutterFaiUpgradePlugin shareInstance].launchOptions Entity:entity completionHandler:^(BOOL granted, NSError *_Nullable error) {
                if (granted) {
            /// 用户允许推送
                    if (callback) {
            ...
            /// 轮询注册deviceToken
                    [[FlutterFaiUpgradePlugin shareInstance] checkDeviceToken];
                } else {
            /// 用户拒绝消息推送
                    if (callback) {
                        callback(@{ @"result": [NSNumber numberWithBool:granted] });
...
static FlutterFaiUpgradePlugin *_instance;
